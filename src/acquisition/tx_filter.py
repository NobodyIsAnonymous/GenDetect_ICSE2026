...
def ethereum_filter():
        ...
        for internal_tx in response['result']:
            ...
            if parent_tx_to not in visited:
                ...
                if parent_tx_to != None and check_address_contract(parent_tx_to) and not check_address_verified(parent_tx_to):
                    out_path = os.path.join(project_root, 'sus_internal_tx.csv')
    ...
    visited_path = os.path.join(project_root, 'visited_addresses.txt')   
...
def dune_filter():
    ...
    csv_path = os.path.join(project_root, 'dune_results_2023.csv')
