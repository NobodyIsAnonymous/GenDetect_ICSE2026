...
def add_benign_rules():
    csv_path = os.path.join(DUNE_DIR, 'classified_tx_filtered.csv')
    ...
    es_files_pattern = os.path.join(DUNE_DIR, '2023', 'new_dune_results_*_100k_*.csv')
    output_dir_path = os.path.join(DUNE_DIR, 'results', '2023/')
    
    replayer = Replayer(es_files=es_files_pattern, output_dir=output_dir_path)
...
def run_benchmark():
    es_files_pattern = os.path.join(DUNE_DIR, '2023', 'new_dune_results_*_100k_*.csv')
    output_dir_path = os.path.join(DUNE_DIR, 'results', '2023/')
    benchmark_data_path = os.path.join(DUNE_DIR, 'benchmark-data.csv')
    ...
    result_path = os.path.join(DUNE_DIR, 'benchmark-results.csv')
...
def cross_validation(verbose=True, sample_size=None):
    ...
    trace_path = os.path.join(DATA_DIR, 'noloop_encoded_trace.csv')
    trace_rules_df = load_data_fixed(trace_path)
