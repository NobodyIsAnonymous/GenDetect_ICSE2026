...
DUNE_DIR = os.path.join(project_root, 'dune_tx')
MODEL_DIR = os.path.join(project_root, 'ml-model') # 定义模型目录
...
def train():
    """ 训练 XGBoost 模型 """
    data_path = os.path.join(DATA_DIR, 'noloop_encoded_trace.csv')
    trace_rules_df = load_data(data_path)
    ...
    best_xgb_clf.save_model(model_json_path)
    joblib.dump(best_xgb_clf, model_pkl_path)
...
class TransactionClassifier:
    ...
    def __init__(self, es_files, output_dir, model_dir):
        # 这里的 es_files 和 output_dir 会传给 Replayer，Replayer 内部如果只接收绝对路径，这里传进去的最好也是绝对路径
        self.replayer = Replayer(es_files=es_files, output_dir=output_dir)
        
        # [修改] 使用 os.path.join 替代 f-string 拼接，防止路径分隔符问题
...
def run_benchmark():
    benchmark_csv = os.path.join(DUNE_DIR, 'benchmark-data.csv')
    benchmark_data = pd.read_csv(benchmark_csv)
    es_files_path = os.path.join(DUNE_DIR, '2023', 'new_dune_results_*_100k_*.csv')
    output_dir_path = os.path.join(DUNE_DIR, 'results', '2023/')
    
    classifier = TransactionClassifier(
        es_files=es_files_path,
        output_dir=output_dir_path,
        model_dir=MODEL_DIR  # [修改] 直接使用全局定义的绝对路径 MODEL_DIR
    )
    results = []
    correct = 0
    total = 0
    ...
    result_csv_path = os.path.join(DUNE_DIR, 'ml-benchmark-results.csv')
    pd.DataFrame(results).to_csv(result_csv_path, index=False)
    print(f"✅ Benchmark 结果已保存至: {result_csv_path}")
