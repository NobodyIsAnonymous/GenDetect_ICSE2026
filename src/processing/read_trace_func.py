...
# 最终生成的 CSV 绝对路径
ATTACK_VECTORS_CSV = os.path.join(DATA_DIR, 'attack_vectors.csv')
...
def create_csv_file():
    ...
    with open(ATTACK_VECTORS_CSV, mode='w', newline='') as attack_vectors_file:
...
def write_csv_file(id, address_vector, function_name_vector, function_params_vector):
    with open(ATTACK_VECTORS_CSV, mode='a', newline='') as attack_vectors_file:
        attack_vectors_writer = csv.writer(attack_vectors_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
...
def pipeline_parse_json_trace():
    ...
    for filename in files:
        ...
        json_trace = read_structured_trace(full_path)
        ...
        # 传参必须也是 filename
        write_csv_file(extract_path_id(filename), address_vector, function_name_vector, function_params_vector)
